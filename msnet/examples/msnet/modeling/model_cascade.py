...
class CascadeRCNNHead(object):
    ...
    def run_head(self, proposals, stage):
        ...
        if self.training:
            refined_boxes = tf.boolean_mask(tensor=refined_boxes, mask=tf_area(refined_boxes) > 0)
    ...
    def match_box_with_gt(self, boxes, iou_threshold):
        ...
        """
        if self.training:
            with tf.compat.v1.name_scope('match_box_with_gt_{}'.format(iou_threshold)):
                iou = pairwise_iou(boxes, self.gt_boxes)  # NxM
                max_iou_per_box = tf.reduce_max(input_tensor=iou, axis=1)  # N
                best_iou_ind = tf.argmax(input=iou, axis=1)  # N
                labels_per_box = tf.gather(self.gt_labels, best_iou_ind)
                fg_mask = max_iou_per_box >= iou_threshold
                fg_inds_wrt_gt = tf.boolean_mask(tensor=best_iou_ind, mask=fg_mask)
    ...
    def losses(self):
        ...
        for idx, head in enumerate(self._heads):
            with tf.compat.v1.name_scope('cascade_loss_stage{}'.format(idx + 1)):
