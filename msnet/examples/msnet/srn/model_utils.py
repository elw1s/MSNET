...
def cal_feature_dist_s(x_1, x_2, axis=0):
    ...
    cos_similarity = tf.reduce_sum(input_tensor=tf.multiply(normalize_a, normalize_b), axis=axis)
...
def cal_feature_dist(x_1, x_2, axis=0):
    ...
    return tf.reduce_sum(input_tensor=final_result) / tf.multiply(tf.cast(x_shape[-3], tf.float32), tf.cast(x_shape[-2], tf.float32))
